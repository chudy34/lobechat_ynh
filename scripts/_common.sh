...
# shared_preload_libraries = 'pg_search' in postgresql.conf) or by a
# previous failed install attempt with a different POSTGRES_PASSWORD.
# A leftover data directory makes pgvector/pgvector skip initdb, so the
# new password generated by the installer never takes effect and LobeHub
# gets 'password authentication failed for user postgres' (28P01).
#
# NOTE: the "wipe any existing PG_VERSION" branch is install-only. On
# upgrade we only wipe when we detect the ParadeDB-era pg_search preload
# library, because that database was never functional under pgvector
# anyway. A normal upgrade must preserve user data.
...
# Install-only variant: on a fresh install there is no user data to
